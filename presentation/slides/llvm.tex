...
\begin{frame}[fragile]{LLVM}
  ...
  \begin{tikzpicture}[thick]
    ...
    \onslide<2->{
      ...
      \onslide<5->{
        \path (0, +1.1) coordinate [Red, box] (lex) node {Lexer};
        \path (0, -1.1) coordinate [Red, box] (parser) node {Parser};
    ...
    \onslide<3-4>{\path (4, 0) coordinate [bigbox] (ir) node {\Large IR};}
    \onslide<5->{\path (4, 0) coordinate [Red, bigbox] (ir) node {\Large IR};}
    ...
    %%%%%%%%%
    \onslide<-3>{
      \path (8, 0) coordinate [bigbox] (back) node {Backend};
      \draw [->] (opt) -- (back);
    }

    \onslide<4->{
      \path (8, +1.1) coordinate [box] (codegen) node {CodeGen};
      \path (8, -1.1) coordinate [box] (opt2)
            node [align=center] {MD\\Optimizer};
      \draw (opt2)+(0, -1.5) node {\textbf{Backend}};
      \draw [->] (ir) -- (opt2);
      \draw [->] (opt2) -- (codegen);
    }
  \end{tikzpicture}
...
\begin{frame}[fragile]{Clang: IR Generation}
  ...
  \begin{tikzpicture}[very thick]
    ...
    \node (code) at (5, 2.5) [text width=4.3cm, align=justify] {
      \begin{lstlisting}[language=llvm, basicstyle=\fontsize{6.8}{6.8}\ttfamily]
...
define i32 @_Z1fi(i32) #0 {
  ...
  ret i32 %3
    ...
    \draw [->, shorten >= 20pt] (0.75, 2.5) -- (code);
  \end{tikzpicture}
\end{frame}
